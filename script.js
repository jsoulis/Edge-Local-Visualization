...
const updateChart = (dataInput) => {
    ...
    if(data[data.length-1] !== dataInput) {
        data.shift();
        data.push(formatDate(dataInput));
        chart.update();
    }
...
async function asyncUpdate() {
    ...
    let dataUpdate = await fetch("http://localhost:5590/api/v1/tenants/default/namespaces/default/streams/OpcUa5.Sinusoid1/data/last")
    let dataItem = await dataUpdate.json();
    updateChart(dataItem);

    setTimeout(asyncUpdate, 500);
}
